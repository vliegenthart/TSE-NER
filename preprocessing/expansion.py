...
def term_expansion_dataset(numberOfSeeds, name, numberOfIteration, iteration):
    ...
    '''We cluster all terms extracted from the sentences with respect to their embedding vectors using K-means. Silhouette analysis is used to find the optimal
number k of clusters. Finally, clusters that contain at least one of the seed terms
are considered to (only) contain entities the same type (e.g Dataset). '''
    maxcluster=0
    if len(df) >= 9:
...
def term_expansion_method(numberOfSeeds, name, numberOfIteration, iteration):
    if int(numberOfIteration) == 0:
        fileUnlabelled = ROOTHPATH + '/evaluation_filesMet/X_train_' + str(numberOfSeeds) + '_' + str(
            iteration) + '.txt'
    else:
